...
def get_rag_answer(event):
    ...
    try:
        # Step 1: Initialize the RAG index from S3 (and cache it)
        if _rag_index is None:
            if not S3_BUCKET_NAME:
                raise ValueError("S3_BUCKET_NAME environment variable not set.")
            
            print(f"RAG: Downloading and caching knowledge base from S3 bucket: {S3_BUCKET_NAME}.")
            
            # Download and load FAISS index from S3 to the /tmp directory
            s3.download_file(S3_BUCKET_NAME, 'rag_index.faiss', '/tmp/rag_index.faiss')
            _rag_index = faiss.read_index('/tmp/rag_index.faiss')
            
            # Download and load chunks from S3
            s3.download_file(S3_BUCKET_NAME, 'rag_chunks.json', '/tmp/rag_chunks.json')
            with open('/tmp/rag_chunks.json', 'r') as f:
                _rag_chunks = json.load(f)

            print("RAG: Index and chunks loaded successfully from S3.")
