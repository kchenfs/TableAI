...
def handle_dialog(event):
    ...
    # Handle the user's response to an option question
    if session_attrs.get('currentItemToConfigure') and slots.get('OptionChoice') and slots.get('OptionChoice').get('value'):
        ...
        for i, item in enumerate(order_items):
            if item.get('normalized_key') == current_item.get('normalized_key'):
                if 'options' not in item or item['options'] is None:
                    item['options'] = {}
                print(f"Updating item '{item['item_name']}' with option '{option_name_to_set}': '{choice_value}'")
    ...
    if slots.get('OrderQuery') and not session_attrs.get('initialParseComplete'):
        ...
        try:
            ...
            for it in order_items:
                ...
                best_key, _ = _fuzzy_find(norm, menu_lookup, embeddings_cache)
                ...
                if best_key:
                    ...
                    normalized_items.append({
                        ...
                        "quantity": quantity, "options": validated_options, "category": menu_entry.get('category'),
            ...
            traceback.print_exc()
            return close_dialog(event, session_attrs, 'Failed', {'contentType': 'PlainText', 'content': "I had trouble understanding your order. Could you try again?"})
    ...
    if session_attrs.get('parsedOrder'):
        ...
        if not any(item.get('normalized_key') for item in normalized_items):
            return elicit_slot(event, session_attrs, 'OrderQuery', "I'm sorry, I couldn't find any of those items on the menu. Could you please tell me your order again?")
                ...
                for opt_key_norm, opt_meta in entry['options'].items():
                    if opt_meta.get('required'):
                        ...
                        if not is_provided:
                            ...
                            message = f"For your {ni['item_name']}, which {option_name} would you like? Your choices are: {choices_text}."
    ...
    if session_attrs.get('parsedOrder'):
        ...
        for item in final_order_items:
            options_str = ""
            if item.get('options'):
                options_str = " (" + ", ".join(f"{v}" for k, v in item['options'].items()) + ")"
...
def fulfill_order(event):
    try:
        ...
        traceback.print_exc()
        session_attrs = event['sessionState'].get('sessionAttributes', {})
        return close_dialog(event, session_attrs, 'Failed', {'contentType': 'PlainText', 'content': "I encountered an error while finalizing your order."})
