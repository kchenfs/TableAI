...
API_KEY = "AIzaSyBI8q-UnZ5z1sMHAGma8MiSgTcP_vCQgnU"  # Use environment variable for API key
...
with menu_table.batch_writer() as batch:
    for i, item in enumerate(items, 1):
        ...
        # --- MODIFIED: Use ItemName only for the embedding ---
        text_to_embed = item_name.strip()
        
        if not text_to_embed:
            print(f"Skipping empty item {i}")
            continue

        # --- ADDED: Print the text being sent to the model ---
        ...
        print(f"[{i}/{len(items)}] Updated: {item_name}")
